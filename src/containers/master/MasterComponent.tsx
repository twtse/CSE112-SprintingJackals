...
export class MasterComponent extends Component<IMasterComponentProps, IMasterComponentState> {
  ...
  private readonly authourizeService: IAuthorizeService;
  // Constructor
  constructor (props: IMasterComponentProps) {
    super(props);

    this.serviceProvider = new ServiceProvide();
    this.authourizeService = this.serviceProvider.createAuthorizeService();
    this.state = {
      loading: true,
      authed: false,
      dataLoaded: false,
  ...
  componentDidMount () {
    this.authourizeService.onAuthStateChanged((isVerifide: boolean, user: any) => {
      const {
        global,
        clearData,
        loadDataGuest,
