	...
	commentList = () => {
		...
		if (comments) {
			...
			return parsedComments.map((comment, index) => {
				...
				const Author = () => (
					<div>
						<NavLink to={`/${comment.userId!}`}> <span
							className={classes.author}>{comment.userDisplayName}</span></NavLink><span style={{
						fontWeight: 400,
						fontSize: "8px"
		...
		const showComments = (!comments.isEmpty()
			? (
				<Paper elevation={0}
					...
					<CommentListComponent comments={comments!} isPostOwner={this.props.isPostOwner}
										  disableComments={this.props.disableComments} postId={postId}/>
				</Paper>)
			: "");
		const loadComments = ((commentsRequestStatus === ServerRequestStatusType.OK) || !comments.isEmpty() ? showComments :
			<LinearProgress style={this.styles.progressbar} variant="indeterminate"/>);
		/**
		 * Return Elements
		 */
		return (
			<div key={postId + "-comments-group"}>
				<Divider/>
				<div style={commentSlides && commentSlides!.isEmpty && !commentSlides!.isEmpty() ? {display: "block"} : {display: "none"}}>
					<Paper elevation={0} className="animate-top" style={!open ? {display: "block"} : {display: "none"}}>

						<div style={{position: "relative", height: "60px"}}>
							<Button style={this.styles.toggleShowList} fullWidth={true}
									onClick={this.props.onToggleRequest}> {" "}</Button>
