		...
		return (
			<Paper key={userId} elevation={20} className={classNames("grid-cell", classes.paper)}>
				<div style={{
					...
					<div onClick={() => this.props.goTo!(`/${this.props.userId}`)} className="people__name"
					...
					<DialogContent className={classes.dialogContent}>
						<List>
							...
							<ListItem key={`'circleName'-${userId}`} dense className={classes.listItem}>
								...
								<ListItemSecondaryAction>
									<IconButton onClick={this.onCreateCircle}
...
const mapStateToProps = (state: Map<string, any>, ownProps: IUserBoxComponentProps) => {
	...
	let userBox = state.getIn(["user", "info", ownProps.userId]);

	if (userBox.toJS) {
		userBox = userBox.toJS();
	}
